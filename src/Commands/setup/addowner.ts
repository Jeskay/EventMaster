...
export const command: Command = {
    ...
    run: async(client, message, args) => {
        try {
            ...
            if(settings.owners.includes(userId)) throw new CommandError("This user already has owner permissions")
            settings.owners.push(userId);
            await client.database.updateSettings(server.guild, {
                owners: settings.owners
            });
            await message.channel.send({embeds: [client.embeds.ownerAdded(user.displayName)]});
        } catch(error) {
